...
function startSchoolRoom() {
  ...
  // Set up Scene, Camera, and keep track of time for controls
  ...
  // Invisible box that the iframe will be added to
  const officeMaterial = new THREE.MeshBasicMaterial({ wireframe: true });
  const officeGeometry = new THREE.PlaneGeometry(7, 2.5, 1);
  const officeMesh = new THREE.Mesh(officeGeometry, officeMaterial);
  officeMesh.rotation.y = 180 * Math.PI / 180;
  officeMesh.position.set(0, 1.95, -6.2);
  townScene.add(officeMesh);

  // Invisible box around the iframe to trigger events
  ...
  moveMesh.position.set(0, 1.95, -6.1);
  townScene.add(moveMesh);

  // Iframe where our task list will be 
  const element = document.createElement("div");
  const iframe = document.createElement("iframe");
  iframe.src = "../../html-tasks/schoolTodo.html";
  iframe.style.width = "680px";
  iframe.style.height = "235px";
  element.appendChild(iframe);

  // Add iframe to invisible box
  ...
  const animate = function () {
    ...
    // Camera boundaries so you dont clip through walls
